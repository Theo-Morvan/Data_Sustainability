...
def compute_output_costs(distance_matrix_km, flows, maximal_distance):
    outputs_costs = (
        distance_matrix_km.min(axis=0).values
        * (distance_matrix_km.min(axis=0).values > maximal_distance)
        ...
        + 0.05
        * (distance_matrix_km < maximal_distance)
        * distance_matrix_km
        * flows
        / 1000
    )
...
if __name__ == "__main__":
    ...
    for colonne in raw_data.columns:
        try:
            raw_data[colonne] = raw_data[colonne].apply(
    ...
    values = ["x", "y"]
    for value in values:
        df.loc[:, f"center_{value}"] = (
            df.loc[:, f"{value}D"] + df.loc[:, f"{value}F"]
        ) / 2
